...
const Level1Screen = ({ route }) => {
  ...
  const handleOptionPress = async (index, selectedAnswer) => {
    ...
    if (isCorrect) {
      ...
      try {
        await savePointsToDynamoDB(username, updatedPoints);
  ...
  const handleNext = async () => {
    ...
    try {
      ...
      await saveLastAttemptedQuestion(username, nextId);
  ...
  const handlePrevious = async () => {
    ...
    if (previousId >= 1) {
      try {
        ...
        await saveLastAttemptedQuestion(username, previousId);
            ...
            <View style={styles.card}>
              ...
              <View style={styles.progressBarContainer}>
                <ProgressCircle
                  size={62}
                  thickness={8}
                  progress={userPoints / MAX_POINTS} // Set MAX_POINTS to the maximum possible points
                  showsText={true}
